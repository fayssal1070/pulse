...
export default async function NotificationsAdminPage() {
  ...
  const activeOrg = await requireActiveOrgOrRedirect(user.id, { nextPath: '/admin/integrations/notifications' })
  ...
  return (
    <AppShell organizations={organizations} activeOrgId={activeOrg.id} isAdmin={isAdminUser} needsOnboarding={false}>
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <h1 className="text-3xl font-bold text-gray-900 mb-6" data-testid="notifications-admin-title">
            Notification Integrations
          </h1>
          <NotificationsAdminClient organizationId={activeOrg.id} />
        </div>
      </div>
    </AppShell>
  )
